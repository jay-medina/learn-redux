...
/***/ function(module, exports, __webpack_require__) {
	...
	var _MultiCounter = __webpack_require__(168);

	var _MultiCounter2 = _interopRequireDefault(_MultiCounter);

	var _CounterReducer = __webpack_require__(170);
	...
	function increaseCount(index) {
	  _CounterReducer.store.dispatch({ index: index, type: 'INCREMENT' });
	}

	function decreaseCount(index) {
	  _CounterReducer.store.dispatch({ index: index, type: 'DECREMENT' });
	}

	function addCounter() {
	  _CounterReducer.store.dispatch({ type: 'ADD_COUNTER' });
	}

	function removeCounter() {
	  _CounterReducer.store.dispatch({ type: 'REMOVE_COUNTER' });
	}

	function render() {
	  _reactDom2.default.render(_react2.default.createElement(_MultiCounter2.default, { values: _CounterReducer.store.getState(),
	    addCounter: addCounter,
	    removeCounter: removeCounter,
	    increaseCount: increaseCount,
	    decreaseCount: decreaseCount }), document.getElementById('app'));
	}

	_CounterReducer.store.subscribe(render);
	render();
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = _react2.default.createClass({
	  displayName: 'MultiCounter',
	  renderCounter: function renderCounter() {
	    var _this = this;

	    return this.props.values.map(function (value, index) {
	      return _react2.default.createElement(_Counter2.default, { key: index, index: index,
	        value: value,
	        increaseCount: _this.props.increaseCount,
	        decreaseCount: _this.props.decreaseCount });
	    });
	  },
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _underscore = __webpack_require__(184);
	...
	function updateCounter(state, action) {
	  if (_underscore2.default.isNumber(action.index) && action.index < state.length && action.type) {
	    return [].concat(_toConsumableArray(state.slice(0, action.index)), [counter(state[action.index], action)], _toConsumableArray(state.slice(action.index + 1)));
	  }
	...
	function multiCounter() {
	  ...
	  switch (action.type) {
	    ...
	    case 'DECREMENT':
	      return updateCounter(state, action);
	    case 'ADD_COUNTER':
	      return modifyCounter(state, action);
	    case 'REMOVE_COUNTER':
	      return modifyCounter(state, action);
	    default:
	      return state;
	...
	var store = exports.store = (0, _redux.createStore)(multiCounter);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ReducerTest = __webpack_require__(186);
	...
	(0, _ReducerTest2.default)({
	  state: [1],
	  action: { index: 1, type: 'INCREMENT' },
	  result: [1],
	  func: _CounterReducer.multiCounter
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	function runTest(_ref) {
	  var state = _ref.state;
	  var action = _ref.action;
	  var result = _ref.result;
	  var func = _ref.func;

	  var actual = func(state, action);
	  var actual_string = JSON.stringify(actual);
	  var action_string = JSON.stringify(action);
	  var result_string = JSON.stringify(result);

	  if (!_underscore2.default.isEqual(result, actual)) {
	    console.log('\n' + func.name + '(' + state + ', ' + action_string + ') = ' + actual_string + '\n    result = ' + result_string + ' ==> ' + _underscore2.default.isEqual(result, actual));
	  }
	}
